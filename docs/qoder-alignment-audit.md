...
| # | 面 | 项 | 官方行为 | 插件行为 | 证据 | 风险 | 判定 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 41 | 4 | scene 解析与跨场景合并 | 遍历响应里**每一个** scene（配置 scene 排第一），配置 scene 的条目在 `allModels` 里覆盖同 key 的其他 scene，`is_default` 只认配置 scene，配置 scene 缺失时打 warn 但不失败（`JS:105953-105970`）；配置 scene 还会并入 `byok_enterprise` 那一档（`JS:105500-105514`，常量 `JS:106048`），按去重键跳过已有条目。配置 scene = `getClientMetadata().scene`，默认 `"assistant"`（`JS:106064`、`JS:400-402`、`JS:404`） | ~~只取 `assistant \\| chat` 一个 scene，不合并、不并 byok_enterprise、静默回退~~ **已修 + 在线验证**：`models.ts` 现在遍历响应里**每个** scene 合并（配置 scene 排第一、同 key 先见者优先 = 配置 scene 赢），`byok_enterprise` 档并入配置 scene（去重键 = `key`，来源记为 `server_scene: "byok_enterprise"`），配置 scene 缺失时 warn 但不失败。实测真实目录合并后 38 个模型（assistant 16 + quest/nap/qwork/experts 独有键 + 11 个 byok_teams UUID + 1 个 byok_enterprise） |\| resData.chat`（`models.ts:173`）：只取一个 scene，不合并、不并 `byok_enterprise`、不记 `server_scene`；而且 `assistant` 缺失时**静默退到另一个 scene**（`chat` 是旧端点的缩减目录），拿到的是一份不同的目录却毫无提示 | 必须对齐 → **已对齐（真实目录在线验证）** | 中 | 必须对齐 |
...
| 52 | 5 | device token 的刷新端点 | `POST <openapi>/api/v1/deviceToken/refresh`，体 `{refresh_token}`，头 `Content-Type` + `Accept` + `User-Agent: qoder/<version>`（`JS:115104`） | ~~`POST <center>/algo/api/v3/user/refresh_token`，体 `{refreshToken}`（驼峰）~~ **已修 + 在线验证**：改成官方 `POST <openapi>/api/v1/deviceToken/refresh`、体 `{refresh_token}`、去掉 `Authorization`。实测旧端点 403 "Request discarded"（确证已死），新端点对缺 body 返回 400 `refresh_token is required`、对 `{refresh_token:"bogus"}` 返回 400 `must start with drt-`、对 `drt-` 前缀的假 token 返回 401 `User not authenticated` —— 端点活着且体形状被服务端接受。响应改读官方的 `device_token`/`refresh_token`/`expires_at`。失败**不再静默延期**：官方失败抛错（`JS:115105-115107`），插件原先吞掉并加一小时正是死端点长期无人发现的原因，现改为抛出带 status 的真错 | `JS:115104` vs `cosy.ts:119-121`、`oauth.ts:226-283`；`cosy.test.ts:getQoderRefreshURL:"constructs correct global URL"` / `"constructs correct CN URL"` 钉住插件当前这条 URL（**它们钉的是现状，不是正确值**） | 中 | 必须对齐 → **已对齐（端点/体/响应字段在线验证；失败语义对齐官方抛错）** |
| 53 | 5 | device flow 的授权 URL 与轮询 | 授权 URL 带 `client_id`（CLI 用的那个固定 UUID，`JS:114161`、常量 `JS:114225`），参数序 `challenge, challenge_method, nonce, machine_id, client_id`；轮询只发 `Accept`，**只把 404 当 pending**（`JS:114166-114167`），间隔 1000 ms、总期限 300 s（`JS:114225`） | ~~授权 URL 没有 `client_id`、参数序不同；轮询把 202 和 404 都当 pending、间隔 2000ms×90~~ **已修 + 在线验证**：授权 URL 补官方 CLI 的 `client_id`（`e883ade2-e6e3-4d6d-adf7-f92ceff5fdcb`，`JS:114225` 的 `s1Q`）并把参数序改成 `challenge, challenge_method, nonce, machine_id, client_id`；轮询只把 404 当 pending、间隔改 1000ms×300。实测：授权页带不带 `client_id` 都 200 且字节相同（服务端不强制它，补上是为对齐）；轮询端点等待中实测返回 404 `Not found`，与官方"只把 404 当 pending"吻合 | `JS:114159-114167`、`JS:114225` vs `login.ts:113-145` | 中 | 必须对齐 → **已对齐（授权页与轮询端点均在线验证）** |
